...
export const ClippyAssistant: React.FC<ClippyAssistantProps> = ({ onOpenApp, onOpenProperties }) => {
  ...
  if (!isOpen) {
    return (
      ...
      >
        ...
        <span className="text-[11px] pr-1">Need help?</span>
      </button>
    );
  }

  return (
    <div className="fixed bottom-10 right-4 z-[9900] flex flex-col items-end max-w-xs font-sans select-none">
      {/* Speech Bubble */}
      <div className="bg-[#ffffcc] border-2 border-zinc-800 rounded-lg p-3 shadow-2xl relative mb-2 text-xs text-zinc-900 leading-relaxed border-b-3 border-r-3">
        {/* Close Button */}
        <button
          onClick={() => {
            sounds.playClick();
        ...
        <p className="font-medium pr-4 mb-2.5">{currentTip}</p>

        {/* Action Suggestions */}
        <div className="space-y-1 pt-1 border-t border-yellow-300">
          {tips.map((t, idx) => (
            <button
              key={idx}
              onClick={t.action}
        ...
        {/* Bubble Tail pointer */}
        <div className="absolute -bottom-2 right-8 w-0 h-0 border-l-8 border-l-transparent border-r-8 border-r-transparent border-t-8 border-t-[#ffffcc]" />
      </div>

      {/* Animated Clippy Icon */}
      <div className="flex items-center gap-2 pr-6 cursor-pointer" onClick={() => sounds.playClick()}>
        <div className="w-12 h-14 relative flex items-center justify-center filter drop-shadow-md hover:scale-105 transition-transform">
          {/* Paperclip Body SVG */}
          <svg viewBox="0 0 100 120" className="w-full h-full">
            ...
            />
            {/* Googly Eyes */}
            <circle cx="50" cy="40" r="8" fill="white" stroke="#2d3748" strokeWidth="2" />
            <circle cx="52" cy="40" r="4" fill="black" />
            <circle cx="68" cy="40" r="8" fill="white" stroke="#2d3748" strokeWidth="2" />
            <circle cx="70" cy="40" r="4" fill="black" />
            {/* Expressive Eyebrows */}
            <path d="M 44 30 Q 50 25 56 30" fill="none" stroke="#1a202c" strokeWidth="2" strokeLinecap="round" />
            <path d="M 62 30 Q 68 25 74 30" fill="none" stroke="#1a202c" strokeWidth="2" strokeLinecap="round" />
          </svg>
        </div>
      </div>
    </div>
